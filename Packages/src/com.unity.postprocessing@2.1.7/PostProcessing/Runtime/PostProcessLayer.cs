...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    {
        static readonly string UnderwaterKeywords = "_LIGHTCOOKIE";
        /// <summary>
        ...
        public void Render(PostProcessRenderContext context)
        {
            ...
            for (int eye = 0; eye < context.numberOfEyes; eye++)
            {
                ...
                bool enableUnderWaterEffect = false;
                foreach (var item in sortedBundles[PostProcessEvent.BeforeStack])
                {
                    var effect = item.bundle;
                    if (effect.renderer.GetType() == typeof(UnderwaterRenderer))
                    {
                        enableUnderWaterEffect = effect.settings.IsEnabledAndSupported(context);
                    }
                }
                cmd = context.command; 
                if (enableUnderWaterEffect)
                {
                    cmd.EnableShaderKeyword(UnderwaterKeywords);
                }
                else
                {
                    cmd.DisableShaderKeyword(UnderwaterKeywords);
                }
